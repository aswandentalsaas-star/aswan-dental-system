...
// 2. دالة البحث المحدثة التي تستخدم الاحتمالات
export async function searchPatients(query: string) {
  try {
    // إذا كان البحث فارغاً، لا نفعل شيئاً
    if (!query.trim()) return [];

    // توليد كل احتمالات الكلمة (مثلاً: روى، رؤى، روي)
    const variations = generateSearchVariations(query);

    // تجهيز شروط البحث بالاسم لكل الاحتمالات
    const nameConditions = variations.map((v) => ({
      name: { contains: v }
    }));
    ...
    const patients = await prisma.patient.findMany({
      where: {
        OR: [
          ...nameConditions,              // ابحث بكل احتمالات الاسم
          { phone: { contains: query } }, // أو ابحث برقم الهاتف
        ],
      },
      take: 5,
    });
